...
class BaseAgent:
    ...
    def remember(self, lesson: str) -> None:
        """Yangi darsni Lessons Learned bo'limiga qo'shadi (MEMORY_WRITE=1 bo'lsa)."""
        if os.getenv("MEMORY_WRITE") != "1":
            return
        p = AGENCY_ROOT / self.memory_file
        if not p.exists():
            return
    ...
    def act(self, task: Task) -> Result:
        ...
        if self.dry_run or not os.getenv("ANTHROPIC_API_KEY"):
            return Result(True, f"[{self.id}] bajarildi: {task.title}", "dry-run")
        return self._act_llm(task)

    def _act_llm(self, task: Task) -> Result:
        """Real rejim — Claude API orqali (ANTHROPIC_API_KEY kerak)."""
        import anthropic  # kechiktirilgan import
        client = anthropic.Anthropic()
        system = f"{self.recall()}\n\nSen {self.role} rolidasan. Missiya: {self.mission}"
        msg = client.messages.create(
            model=self.model, max_tokens=1500,
            thinking={"type": "adaptive"},
            system=system,
            messages=[{"role": "user", "content": f"Vazifa: {task.title}\n{task.payload}"}],
        )
